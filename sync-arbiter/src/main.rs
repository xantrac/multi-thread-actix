...
impl Handler<SlowMessage> for SlowProcessor {
    ...
    fn handle(&mut self, msg: SlowMessage, _ctx: &mut SyncContext<Self>) -> Self::Result {
...
impl Handler<FastMessage> for FastDisplayer {
    ...
    fn handle(&mut self, msg: FastMessage, _ctx: &mut SyncContext<Self>) -> Self::Result {
...
async fn main() {
    ...
    let fast_addr = SyncArbiter::start(2, || FastDisplayer);

    let slow_addr = SyncArbiter::start(2, || SlowProcessor);
    let res = slow_addr
        .send(SlowMessage {
